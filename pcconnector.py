...
class PostCardUser(object):
    ...
    def __init__(self, login, password):
        """
        Initialising of urllib2
        Commented lines for using debug http proxy with SSL
        """
        self.login = login
    ...
    def gettoken(self):
        """
        Getting CSRS token to bypass security check.
        """
    ...
    def loginToPC(self):
        """
        Getting coockie/session, using our credentials.
        """
        ...
        response = self.opener.open("https://www.postcrossing.com/login",
                                    login_data)
        page = ''.join(response.readlines())
        regex = r'send\">(\d) left'
        result = re.search(regex, page)
